...
const changeButtonBackground = (e) => {
    e.target.style = 'background:#000000;color:#ffffff;';
}

sayHelloButton.addEventListener('mouseover', changeButtonBackground);
...
// CODE HERE
const resetButtonBackground = (e) => {
    e.target.style = 'background:#efefef;color:#000000';
}

sayHelloButton.addEventListener('mouseout', resetButtonBackground);
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get(`http://localhost:3000/repeat/myparameter`)
    .then(res => {
...
// CODE HERE
const query = () => {
    axios.get(`http://localhost:3000/query-test?me=Eric&you=Ted`)
    .then(res => {
        console.log(res.data);
    }).catch(error => {
        console.log(error);
...
//1. The message 'You sent an empty query' appeared in the console
//2. An object was displayed in the console along with the message 'You sent more than 1 query'
...
const createFood = () => {
    ...
    .then(res => {
        let foodList = document.querySelector('ul#food-list');
        removeChildren(foodList);
        res.data.map(food => {
...
let addFoodForm = document.querySelector('#add-food');
